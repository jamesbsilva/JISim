...
*/
__kernel void ising2d_longmetroboost(__global int* spins,__global const int* fixed, __global const float* Params, __global float* random, __global int* dM, __global int* spinSums, __global float* dE, const int numThreads, const int nRuns,float temp, __global float* errorBuff) {
	...
	int isSumPos=0;
        ...
        for(int currRun=0; currRun < nRuns; currRun++ ){
            ...
            if(s0 >= 0){isCurrPos=1;}
            if(spinSums[i0]>=0){isSumPos=1;}

            index = (isCurrPos+(isSumPos*4)+abs(spinSums[i0]*4*2));

            delE = 2*s0*(spinSums[i0]*jInteraction+hField);

            // Check to accept flip or not    	
            if((delE <= 0) || (exp(-1*delE/temperature) > random[tId+(2*currRun+1)*numThreads])){
                    // accepted flip
