/* 
    ...
    static spins. Work is split using spatial division . 


    James B. Silva <jbsilva@bu.edu>
*/
__kernel void ising2d_longmetro(__global int* spins,__global const int* fixed, __global const float* Params, __global float* random, __global int* dM, __global float* dE, const int numThreads, const int nRuns, __global float* errorBuff) {
	...
	int isSumPos=0;
        ...
        for(int currRun=0; currRun < nRuns; currRun++ ){
            ...
            index = (tId+(2*currRun)*numThreads);
