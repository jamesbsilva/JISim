...
* @since       2013-10
*/
public class SqInteractionMap extends InteractionMap {
    public SqInteractionMap(int R, int dim){
        super();
        if(R > 0){
            initMap(R,dim);
        // nearest neighbor
    ...
    private void initMap(int R , int dim){
        if(dim > 3){
            throw new IllegalArgumentException("Dimension greater than 3 not allowed.");
        }
        ArrayList<int[]> tempIn  = new ArrayList<int[]>();
        int subL = 2*R + 1 ;
        int m, n, p;
